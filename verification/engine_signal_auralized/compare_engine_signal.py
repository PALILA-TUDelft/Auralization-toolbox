...
# Configuration
folder = "verification/engine_signal_auralized"
fs = 48000
pref = 20e-6
window_size = 1024
overlap = 0.75
...
# --- Load signals ---
matlab_data = loadmat(os.path.join(folder, "engine_signal_matlab.mat"))
signal_matlab = matlab_data["auralizedEngineSignal"].squeeze()

python_data = np.load(os.path.join(folder, "engine_signal_python.npz"))
...
fig.suptitle(f"Engine Signal - Spectrogram Comparison\nMean SPL Abs Error = {mae_spl:.2f} dB", fontsize=14)
...
for ax, title, spl, cmap in zip(axs, titles, data, cmaps):
    ...
    fig.colorbar(im, cax=cax).set_label('SPL [dB re 20 µPa]', fontsize=10)
